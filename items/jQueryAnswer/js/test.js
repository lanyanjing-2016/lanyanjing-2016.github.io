var data = {
    ...
    "exam": [
        ...
        {
            "title": "阅读理解",
            "infos": "1950年的一个夜晚，北极夜空上方出现淡红和淡绿色的光弧，时而像在空中舞动的彩带，时而像在空中燃烧的火焰，时而像选在天边的巨伞……它轻盈的飘荡，不断变化着自己的形状，持续了几个小时。她多彩多姿，一会儿红，一会儿蓝，一会儿绿，一会儿紫，变幻莫测。这就是美丽的极光。",
            "values": [
                {
                    "type": 7,
                    "questionStem": "极光的速度是多少________千米/小时",
                    "options": [
                        "200",
                        "400",
                        "300",
    ...
    if(test_id!=""){
        //$.ajax({
            ...
            $.each(exam, function(h, exam) {
                ...
                $.each(test, function(i, topic) {//1单选2多选3判断4填空5问答6完型填空7阅读理解8论述题
                    var type = topic.type;
                    ...
                    if(type==1){ 
                        $.each(options, function(j, option){
                            ...
                            option_box +=`<div class="jxz-option radio">`
				                        +`<label><input name="test`+h+``+i+`" type="radio" value="`+op+`"> `+op+`：`+option+`</label>`
				                        +`</div>`;
                        }); 
                    }else if(type==2){
                        //answer_op = ``;
                        $.each(options, function(j, option) {
                            var op = convert(j);
                            option_box +=`<div class="jxz-option checkbox">`
                                        +`<label>`
